...
安全网关自身开销极小，主要延迟来自可选的大模型判定引擎。以下为 Windows 单实例实测（`perf_test.py`，本地部署，100 次采样）：

| 场景 | P50 | 平均 | 说明 |
|---|---|---|---|
| 普通输入放行 | 1.7 ms | 7 ms | 纯规则快速路径，无大模型 |
| 工具调用放行 / 拦截 | 1.4 ms | 5.4 ms | 白名单 + 参数校验 + JWT 令牌 |
| 输出脱敏 + 水印 | 1.6 ms | 4.8 ms | 脱敏 + 零宽水印 + 差分隐私 |
| 大模型判定（可选） | ~0.9 s | ~0.9 s | 仅可疑内容 / 强制审查触发；30 秒判断缓存命中零开销 |

- **网关自身开销 < 2 ms**，对业务几乎无感；P95 的 20+ ms 主要来自测试客户端每次新建 HTTP 连接，业务侧使用长连接会更低
- **并发能力**：20 并发实测吞吐约 1470 请求/秒（单实例）
