...
安全网关自身开销极低，主要延迟来自可选的大模型判定引擎。以下为 Windows 单实例实测（`perf_test.py`，本地部署、**keep-alive 长连接**、100 次采样）：

| 场景 | P50 | 平均 | P95 | 说明 |
|---|---|---|---|---|
| 普通输入放行 | 0.9 ms | 0.9 ms | 1.2 ms | 纯规则快速路径，无大模型 |
| 工具调用放行 / 拦截 | 1.0 ms | 1.1 ms | 1.5 ms | 白名单 + 参数校验 + JWT 令牌 |
| 输出脱敏 + 水印 | 1.2 ms | 1.3 ms | 1.9 ms | 脱敏 + 零宽水印 + 差分隐私 |
| 大模型判定（可选） | ~0.9 s | ~0.9 s | — | 仅可疑内容 / 强制审查触发；30 秒判断缓存命中零开销 |

- **网关自身开销约 1 ms，平均与中位基本一致**（长连接接入时）
- **注意连接方式**：若每次请求新建 TCP 连接，会叠加约 20ms 握手开销（P95 抬到 20+ms），属客户端问题；业务侧使用连接池 / keep-alive（requests.Session、http.client 复用、axios、OKHttp 等）即可消除
- **并发能力**：20 并发实测吞吐约 1280 请求/秒（单实例）
- **全程可观测**：响应含 `latency_ms` / `llm_ms` 字段；审计日志逐条记录耗时；CSV 报表含"总耗时(ms) / 模型耗时(ms)"列
- 大模型判定是唯一显著开销（约 0.9 s/次），可通过配置关闭、切换本地/云端引擎或依赖 30 秒判断缓存降低
