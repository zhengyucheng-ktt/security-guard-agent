...
def blocks_qa():
    return [
        ...
        Spacer(1, 4 * mm),
        card("安全自测（三套测试集：基础 / 自动变体 / 语义级）", [
            "基础攻击 61 个：本地 55/61（90%）→ 混合 57/61（93%）",
            "自动变体 98 个（同义词/编码/角色/谐音自动生成）：本地 91/98（93%）→ 混合 98/98（100%）",
            "语义级专项 20 个（角色扮演/隐喻/社工）：本地 15/20（75%）→ 混合 16/20（80%）——剩余为纯隐喻与模糊社工，属 AI 安全公认边界",
            "90 个正常用户样本：开启自动改写后误拦截 0/90（0%）——用户报手机号/身份证自动脱敏放行（132****5678）；修复 10 个真实漏洞（编码解码/混淆/参数注入/思维链等）",
        ]),
        Spacer(1, 4 * mm),
        card("PRD 核心验收标准（本地 vs 混合）", [
            "越狱识别率 ≥ 98%：本地 14/15（93%）⚠️ → 混合 15/15（100%）✅　·　高危工具 100%：12/12 ✅（两种模式均达标）",
            "实质性攻击拦截：本地 55/57（96.5%）⚠️ → 混合 57/57（100%）✅　·　误拦截率 ≤ 1%：开启自动改写后实测 0/90（0%）✅（两种模式均达标）",
            "多轮诱导按组合识别 ≥ 95%：5 轮实测联动拦截 ✅　·　日志 100% 可溯源（哈希链可校验）✅",
        ]),
    ]
