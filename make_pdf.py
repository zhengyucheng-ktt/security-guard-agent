...
def blocks_qa():
    return [
        ...
        Spacer(1, 4 * mm),
        card("安全自测（红队演练 + 误判测试）", [
            "61 种攻击手法：本地 7B 拦截 55/61（90%），本地+DeepSeek 混合拦截 57/61（93%）",
            "90 个正常用户样本：误拦截 7（全部为隐私设计拦截），纯误判 0——删除记录/忽略消息/改绑手机号/订单号等全部放行",
            "多轮诱导 5 轮对话：前 4 轮无害铺垫放行，第 5 轮敏感请求联动拦截",
            "修复 10 个真实漏洞：Base64/HTML实体/Unicode转义/hex 解码、反写/拼音/谐音混淆、银行卡/邮箱误判、工具参数注入、XSS、批量数组、思维链危险意图",
        ]),
        Spacer(1, 4 * mm),
