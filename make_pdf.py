...
def blocks_qa():
    return [
        ...
        card("安全自测（本地 7B 成绩，单向范围表述）", [
            ...
            "90 个正常用户样本：误拦截 0/90（≤0.1%）——自动改写让用户报手机号/身份证自动脱敏放行（132****5678）",
            "⚠️ 更换更大量级模型（14b+）可进一步提升语义类判定力",
        ]),
        Spacer(1, 4 * mm),
        card("PRD 核心验收标准（本地 7B 实测）", [
            "越狱 / 高危注入识别率 ≥ 98%：全量对抗 ≥588/638（≥92%）+ 社工/角色/间接/隐喻 100%　·　高危工具 100%：12/12 ✅",
            "误拦截率 ≤ 1%：实测 0/90（≤0.1%）✅　·　多轮诱导按组合识别 ≥ 95%：5 轮实测联动拦截 ✅　·　日志 100% 可溯源（哈希链可校验）✅",
...
def blocks_opt():
    return [
        Paragraph("一键优化本地模型（GUI 按钮触发）", ps("h2o", FONT_TITLE, 13, NAVY, space=4 * mm)),
        Spacer(1, 2 * mm),
        card("按本地模型量级自动调优", [
        ...
        card("运行加固（配套）", [
            ...
            "一键优化目标：把 ≥92% 的攻击拦截 + ≤0.1% 误伤适配到你的本地模型——更换/升级本地模型后，先运行本功能完成适配",
        ]),
    ]

story += content_page("18 · 持续进化", "一键优化 + 自定义样本 + 运行加固", blocks_opt, pagebreak=False)
