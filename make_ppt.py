...
toc = [
    ...
    ("08", "持续进化", "智能调优（dry-run 建议）· 自定义样本 · 运行加固"),
    ("08.5", "部署硬件参考", "本地模型档位与显卡配置阶梯（TCO 决策）"),
    ("09", "部署与交付", "绿色 ZIP / Docker / 多实例（单实例零依赖）"),
]
for i, (no, t, d) in enumerate(toc):
    y = 1.4 + i * 0.6
    add_text(s, 1.3, y, 1.2, 0.5, no, 18, GOLD, True)
    add_text(s, 2.7, y + 0.02, 4.2, 0.5, t, 15, NAVY2, True)
    add_text(s, 7.2, y + 0.04, 5.2, 0.4, d, 11.5, GREY)
...
s = prs.slides.add_slide(BLANK)
title_bar(s, "03 · 输入过滤（Inbound Filtering）", "四道关卡：规则拦截 · 编码归一 · 多轮语境 · 低风险改写")
...
# ===== 8 输出防线 =====
s = prs.slides.add_slide(BLANK)
section(s, "03", "输出脱敏与溯源（Output Sanitization）", "脱敏 + 水印 + 差分隐私，三层防护", [
    ("动态分级脱敏（10 类信息）", [
        "手机 / 身份证 / 银行卡 / 邮箱 / IP / 姓名",
        "地址 / 车牌 / 营业执照 / 微信号",
...
# ===== 9 反刷评 =====
s = prs.slides.add_slide(BLANK)
title_bar(s, "03 · 反刷评与账号风控", "让机器灌水、批量薅羊毛无处遁形")
add_text(s, 0.7, 1.55, 11.9, 0.35, "反刷评作用于请求入口阶段（先去重/限流/信誉分，再进入输入过滤与判定引擎），与输入防线协同", 11.5, GREY, False, PP_ALIGN.CENTER)
card(s, 1.0, 2.0, 5.55, 4.55, "四重反刷机制", [
...
card(s, 6.78, 2.0, 5.55, 4.55, "会话风险积分（自动升级）", [
...
for i, (act, judge, result, is_block) in enumerate(demo_rows):
    bg = LIGHT if i % 2 == 0 else WHITE
    add_rect(s, 1.0, yy, 11.3, 0.62, bg, BORDER)
    add_text(s, 1.2, yy + 0.07, 4.6, 0.48, act, 11.5, DARK, False)
    add_text(s, 6.0, yy + 0.07, 3.8, 0.48, judge, 11.5, DARK, False)
    add_text(s, 10.0, yy + 0.07, 2.2, 0.48, result, 11.5, RGBColor(0xC0, 0x39, 0x2B) if is_block else GREEN, True)
    yy += 0.7
card(s, 1.0, 5.85, 11.3, 1.35, "每个判定都可追溯", [
...
add_rect(s, 6.78, 1.65, 5.55, 2.75, NAVY, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
add_text(s, 6.78, 1.72, 5.55, 0.5, "PRD 核心验收标准（按模式达标）", 14, GOLD, True, PP_ALIGN.CENTER)
prd = [
    ("越狱/高危注入识别", "标准 ≥98% / 纯净 ≥92%", "达标"),
...
for name, req, got in prd:
    add_text(s, 6.95, yy, 2.85, 0.4, name, 10, WHITE, False)
    add_text(s, 9.85, yy, 1.85, 0.4, req, 8, GOLD2, True)
    add_text(s, 11.75, yy, 0.55, 0.4, got, 10, RGBColor(0x8F, 0xE0, 0xA8), True)
    yy += 0.48
# 底部：开箱即测示例
...
add_text(s, 1.0, 1.72, 5.55, 0.4, "本地 7B · 攻击拦截", 17, GOLD, True, PP_ALIGN.CENTER)
add_text(s, 1.0, 2.15, 5.55, 1.0, "≥92%", 44, WHITE, True, PP_ALIGN.CENTER)
add_text(s, 1.0, 3.3, 5.55, 0.5, "全量对抗 588/638 · 大模型攻击 55/60（≥92%）", 12.5, GOLD2, False, PP_ALIGN.CENTER)
add_text(s, 1.0, 3.85, 5.55, 0.35, "变体 98/98（100%）· 语义 20/20（100%）· 红队 67/73（≥92%）", 10.5, GOLD2, False, PP_ALIGN.CENTER)
# 中块：误伤
add_rect(s, 6.78, 1.65, 5.55, 2.55, GREEN, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
add_text(s, 6.78, 1.72, 5.55, 0.4, "本地 7B · 误伤保护", 17, WHITE, True, PP_ALIGN.CENTER)
add_text(s, 6.78, 2.15, 5.55, 1.0, "0 误伤", 40, WHITE, True, PP_ALIGN.CENTER)
add_text(s, 6.78, 3.3, 5.55, 0.5, "正常对话/业务抽测 82 样本 0 误伤", 12, WHITE, False, PP_ALIGN.CENTER)
add_text(s, 6.78, 3.85, 5.55, 0.35, "POC 阶段误伤率预估 <1%；正式版目标 ≤0.1%", 9, WHITE, False, PP_ALIGN.CENTER)
...
# ===== 17 智能调优与自定义样本（新功能） =====
s = prs.slides.add_slide(BLANK)
title_bar(s, "08 · 持续进化 · 智能调优", "按本地模型量级自动生成建议：三项测试 + 误伤安全阀 + dry-run 确认")
# 左：流程
...
opt_steps = [
    ...
    ("④ 生成建议关键词", "穿透样本提取 → 逐个过误伤测试（dry-run）"),
    ("⑤ 管理员确认落盘", "建议先出差异报告，人工确认后才写入配置生效"),
]
yy = 2.3
...
card(s, 1.0, 5.15, 11.3, 1.55, "配套运行加固", [
    ...
    "~调优仅生成建议（dry-run）并输出差异报告，由管理员确认后落盘——规则变更走「沙箱验证→人工审核→确认发布」，避免误杀生产",
])
s = prs.slides.add_slide(BLANK)
# ===== 18 硬件配置阶梯（TCO 决策参考，08 章节子页） =====
title_bar(s, "08.5 · 部署硬件参考", "本地模型档位与显卡配置阶梯（TCO 决策）")
hw = [
...
yy = 2.0
for name, vram, gpu, note in hw:
    add_rect(s, 1.0, yy, 11.3, 1.0, LIGHT if yy % 2 == 0 else WHITE, BORDER)
    add_text(s, 1.2, yy + 0.12, 3.2, 0.7, name, 13, NAVY2, True)
    add_text(s, 4.6, yy + 0.15, 2.0, 0.5, vram, 12, DARK, False)
    add_text(s, 6.7, yy + 0.15, 3.0, 0.5, gpu, 10.5, DARK, False)
    add_text(s, 9.8, yy + 0.12, 2.4, 0.7, note, 9.5, GREY, False)
    yy += 1.1
add_text(s, 1.0, 6.6, 11.3, 0.5, "低配可选 3b（≥4GB）；无显卡可用 CPU 推理（判定 ~5-10s，仅测试用）——推荐 7b 起步，强对抗场景用云端/混合", 11, GREY, False, PP_ALIGN.CENTER)
s = prs.slides.add_slide(BLANK)
# ===== 19 部署交付 =====
