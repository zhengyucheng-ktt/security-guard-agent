...
toc = [
    ...
    ("06", "性能与安全自测", "约 1ms 开销、90% 拦截、误判均为隐私设计"),
...
add_text(s, 1.0, 3.25, 5.55, 0.6, "拦截 55 / 61 种攻击手法", 13, GOLD2, False, PP_ALIGN.CENTER)
add_text(s, 1.0, 3.65, 5.55, 0.5, "越狱 14/15 · 工具 12/12 · 思维链 5/5 · 混淆 15/15", 11, GOLD2, False, PP_ALIGN.CENTER)
...
card(s, 1.0, 4.45, 11.3, 2.4, "演练成果与边界说明", [
    "修复 10 个真实漏洞：Base64（含双层）/ HTML 实体 / Unicode 转义 / hex 解码、反写/拼音/谐音混淆、银行卡/邮箱误判、工具参数注入、XSS、批量数组、思维链危险意图",
    "剩余穿透：3 个多轮铺垫单条（设计正确，靠语境分联动）· 银行卡中性串（防订单号误判取舍）· 2 个语义盲区（本地模型判定边界，接云端更强模型可兜底）",
