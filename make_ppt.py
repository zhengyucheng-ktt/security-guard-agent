...
toc = [
    ...
    ("07", "质量门禁与验收标准", "67 项测试 · PRD 达标对照 · 开箱即测"),
...
card(s, 6.78, 3.35, 5.55, 3.3, "防线详解", [
...
# 混合判定流程图
add_text(s, 0.7, 1.55, 11.9, 0.4, "混合模式判定流程（本地初筛 → 云端终审）", 14, NAVY2, True, PP_ALIGN.CENTER)
flow_row(s, 0.7, 2.0, 11.9, 1.0, ["可疑内容触发", "本地 7B 初筛", "判安全→云端终审", "DeepSeek 判定", "拦截 / 放行"], size=12)
...
add_text(s, 1.2, 3.64, 3.4, 0.45, "模式", 13, WHITE, True)
...
card(s, 1.0, 6.4, 11.3, 0.9, "失败策略", [
    "fallback 自动降级另一引擎（推荐）· block 直接拦截（最安全）· allow 直接放行（有风险）",
])
...
# ===== 16 质量门禁与验收标准 =====
s = prs.slides.add_slide(BLANK)
title_bar(s, "07 · 质量门禁与验收标准", "Quality Gates · 交付质量 + PRD 硬性指标 + 开箱即测")
# 左：交付验证
...
for name, req, got in prd:
    ...
    yy += 0.43
add_text(s, 6.78, 4.25, 5.55, 0.3, "本地模式已达 93%+，混合模式全面达标", 10, GOLD2, False, PP_ALIGN.CENTER)
# 底部：开箱即测示例
