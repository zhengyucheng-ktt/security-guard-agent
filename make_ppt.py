...
toc = [
    ...
    ("06", "性能与安全自测", "1ms 开销 · 本地 vs 混合实测对比 · 越狱达标"),
    ("07", "质量门禁与验收标准", "自动化测试 · PRD 达标对照 · 开箱即测"),
...
# ===== 7 工具防线（流程图 = 框架，卡片 = 细节，不重复） =====
s = prs.slides.add_slide(BLANK)
title_bar(s, "03 · 工具防线：管住 AI 能做的事", "四步把关，高危操作层层设卡")
flow_row(s, 0.7, 1.6, 11.9, 1.05, ["① 白名单授权", "② 参数清洗", "③ 深度校验", "④ 授权令牌"], size=13)
card(s, 1.0, 2.9, 5.55, 3.7, "每一步的细节", [
    "白名单外一律拦截；高危工具（删除 / 转账 / 提权）必拦",
    "按允许键过滤，未声明字符串键消毒剔除",
    "检测 SQL / 路径遍历 / XSS / 敏感参数",
    "数组批量 ≥5 项视为批量操作",
    "签发 30 秒有效 JWT，工具端自证授权",
    "~任一环节不通过即拦截并记审计",
])
card(s, 6.78, 2.9, 5.55, 3.7, "工具结果回传也设防", [
    "网页 / 文档返回内容可能携带恶意指令",
    "进模型上下文前先扫描（间接注入）",
    "命中即拦截，防止污染模型上下文",
    "思维链同步检测 AI 自主危险意图",
    "~在动手前拦截；高危工具拦截率实测 12/12",
])
