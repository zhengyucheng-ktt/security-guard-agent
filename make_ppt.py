...
toc = [
    ...
    ("06", "性能与安全自测", "约 1ms 开销、93% 拦截（DeepSeek 云端兜底）"),
...
add_text(s, 1.0, 1.72, 5.55, 0.6, "攻击拦截率", 17, GOLD, True, PP_ALIGN.CENTER)
add_text(s, 1.0, 2.2, 5.55, 1.0, "93%", 44, WHITE, True, PP_ALIGN.CENTER)
add_text(s, 1.0, 3.25, 5.55, 0.6, "拦截 57 / 61 种攻击手法", 13, GOLD2, False, PP_ALIGN.CENTER)
add_text(s, 1.0, 3.65, 5.55, 0.5, "越狱 15/15 · 工具 12/12 · 思维链 5/5 · 混淆 15/15", 11, GOLD2, False, PP_ALIGN.CENTER)
add_rect(s, 6.78, 1.65, 5.55, 2.55, GREEN, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
add_text(s, 6.78, 1.72, 5.55, 0.6, "误拦截率（90 正常样本）", 17, WHITE, True, PP_ALIGN.CENTER)
add_text(s, 6.78, 2.2, 5.55, 1.0, "7 / 90", 40, WHITE, True, PP_ALIGN.CENTER)
add_text(s, 6.78, 3.25, 5.55, 0.6, "全部为隐私设计拦截", 13, WHITE, False, PP_ALIGN.CENTER)
add_text(s, 6.78, 3.65, 5.55, 0.5, "纯误判 0：日常对话 / 订单号 / 改绑手机号全放行", 11, WHITE, False, PP_ALIGN.CENTER)
card(s, 1.0, 4.45, 11.3, 2.4, "演练成果与边界说明", [
    "判定引擎：本地 7B 初筛 + DeepSeek 云端终审（hybrid），语义级攻击（角色扮演 / 索取他人隐私）也能识别",
    "修复 10 个真实漏洞：Base64 / HTML 实体 / Unicode 转义 / hex 解码、反写/拼音/谐音混淆、银行卡/邮箱误判、工具参数注入、XSS、批量数组、思维链危险意图",
    "剩余穿透：3 个多轮铺垫单条（设计正确，靠语境分联动）· 1 个银行卡中性串（防订单号误判取舍）",
    "多轮诱导 5 轮实测：前 4 轮无害铺垫放行，第 5 轮敏感请求联动拦截；误判全部为隐私设计拦截（纯误判 0）",
])
