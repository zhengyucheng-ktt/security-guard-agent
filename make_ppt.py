...
# ===== 15 安全自测（本地 vs 混合对比） =====
s = prs.slides.add_slide(BLANK)
title_bar(s, "06 · 安全自测", "本地 7B vs 本地+DeepSeek 混合，双向验证（61 攻击样本 / 90 正常样本）")
# 左块：本地 7B
...
add_text(s, 1.0, 3.25, 5.55, 0.6, "拦截 55 / 61 · 越狱 14/15", 13, GOLD2, False, PP_ALIGN.CENTER)
add_text(s, 1.0, 3.65, 5.55, 0.5, "语义级盲区 2 个：角色扮演 / 索取他人隐私", 11, GOLD2, False, PP_ALIGN.CENTER)
# 右块：混合
...
add_text(s, 6.78, 3.25, 5.55, 0.6, "拦截 57 / 61 · 越狱 15/15", 13, WHITE, False, PP_ALIGN.CENTER)
add_text(s, 6.78, 3.65, 5.55, 0.5, "语义级攻击全部拦截（云端兜底）", 11, WHITE, False, PP_ALIGN.CENTER)
card(s, 1.0, 4.45, 11.3, 2.4, "达标对照（按'实质性攻击'口径：剔除多轮铺垫单条与防误判取舍）", [
    "越狱识别率 ≥98%：本地 14/15（93%）⚠️ → 混合 15/15（100%）✅　·　高危工具 100%：12/12 ✅（两种模式均达标）",
    "实质性攻击拦截：本地 55/57（96.5%）⚠️ → 混合 57/57（100%）✅　·　误拦截率 ≤1%：纯误判 0/90 ✅（两种模式均达标）",
    "多轮诱导按组合识别：5 轮实测前 4 轮铺垫放行、第 5 轮敏感请求联动拦截 ✅",
    "结论：本地模式已覆盖规则层与大部分语义攻击；混合模式补上最后的语义级盲区，全面达标",
])
